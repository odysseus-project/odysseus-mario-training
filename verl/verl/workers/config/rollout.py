...
class MultiTurnConfig(BaseConfig):
    ...
    markov_context_turns: Optional[int] = None
    markov_discount_factor: Optional[float] = None
    group_interval: int = 1
    norm_in_agent_loop: bool = False
    format_penalty: float = -100.0
    # Defer pixel_values creation to training time (aggressive memory optimization)
    # When True: store only PIL images during rollout, create pixel_values during training
    # When False: store pre-computed pixel_values during rollout (original behavior)
    defer_pixel_values: bool = False
    image_resize_ratio: int = 8
...
class AgentLoopConfig(BaseConfig):
    ...
    update_world_level_weights: bool = False
