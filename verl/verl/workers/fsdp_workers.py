...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            # Load processor for deferred pixel_values creation (memory optimization)
            processor = None
            defer_pixel_values = self.config.get("rollout", {}).get("multi_turn", {}).get("defer_pixel_values", False)
            if defer_pixel_values:
                from verl.utils import hf_processor
                processor = hf_processor(local_path, trust_remote_code=self.config.model.get("trust_remote_code", False))
                if self.rank == 0:
                    print(f"[defer_pixel_values] Loaded processor for deferred pixel_values creation")
